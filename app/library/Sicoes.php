...
class Sicoes {
    ...
    /**
     * obtiene los grupos por semestre.
     *
     * @param  int $periodo|required
     *
     * @param  int $semestre|required
     ...
     * @return App\Models\Sicoes\EncGrupo
     */
    public static function getGroupsBySemestre($periodo, $semestre)
    {
        return EncGrupo::where("PeriodoId", $periodo)
                            ->where("Semestre", $semestre)
                            ->get();
    }

    /**
     * Obtiene la ultima inscripcion por periodo y por alumno id
     *
     * @param  int $id_alumno|required
     *
     * @return App\Models\Sicoes\Inscripcion
     */
    public static function validateLastInscripcionByPeriod($id_alumno)
    {
        $config = getConfig();
        $inscription = Inscripcion::where("AlumnoId", $id_alumno)
                                    ->where("PeriodoId", $config->period_id)
    ...
    *
    ...
    public static function currentGroup($id_alumno) {
        $query = Inscripcion::leftJoin("EncGrupo as e", "Inscripcion.EncGrupoId", "e.EncGrupoId")
                ->where("Inscripcion.AlumnoId", $id_alumno)
                ->orderBy("InscripcionId", "desc")
                ->first();
        return $query;
    }

    public static function getAlumnPeriods($alumn_id)
    {
        $period = Carga::leftJoin("Periodo as p", "p.PeriodoId", "Carga.PeriodoId")
                        ->select("p.Clave")
                        ->where("AlumnoId", $alumn_id)
                        ->groupBy(["p.Clave"]);
        return $period->get();
    }

    public static function getMatriculaTemp() {

       
        return $data;
    }

    public static function generateTempMatricula() {

        $data = Alumno::where("Matricula", "like", 'Aspirante%')
                ->orderBy("AlumnoId", "desc")
                ->first();
        if ($data) {
            $tempNumber = substr($data->Matricula,9)+1;
